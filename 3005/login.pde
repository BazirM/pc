...
PImage login_image,start_image,game_image,wait_image,ranking_image;
PFont f;
boolean OK = true;
...
void setup() {
  ...
  wait_image = loadImage("medieval.gif");
  game_image = loadImage("jogo.png");
  ranking_image = loadImage("ranking.png");
...
if(st.game_window)
  ...
  fill(255,86,0);
  textSize(40);
  text("Matchmaking...",-120,120);
  for(int i=0;i<n;i++){
    cur[i]=(float)(count[i]-i*delta)/spd;
    ellipse(R*cos(cur[i]),R*sin(cur[i]),d,d);
    if(((int)(cur[i]/PI))%2==0){
      count[i]++;
    }else{
      count[i]+=10;
...
void showGame(){
 ...
 if(st.game_over){
        ...
        showMessageDialog(null,"Game over!\nPlayer: "+names[0]+" Score: "+score[0][0]+"\nPlayer: "+names[1]+" Score: "+score[1][0], "Alert", ERROR_MESSAGE);
        st.game_over = false;
...
void showRanking(){
  image(ranking_image, 0, 0, width, height);
  cp5.getController("LOGIN").hide();
  cp5.getController("CLOSE ACCOUNT").hide();
  cp5.getController("CREATE AVATAR").hide();
  cp5.getController("USERNAME").hide();
