...
public class Status{
    ...
    public void updateScore(String username, double loserScore, double winnerScore){
      ...
      for (Map.Entry<Player,Avatar> entry : online.entrySet()){
            if(entry.getKey().getUsername().equals(username)){
              p = entry.getKey();
              System.out.println("EStou aqui no updateScore do "+p);
    ...
    public void rankingLevel(String username, int level){
      ...
      try {
        ...
        for(Level l : rankingLevel)
          System.out.println("RankingLEVEL: User: "+l.getUsername()+" Level: "+l.getLevel());
      }
      finally {
        l.unlock();
      }
