		...
		{online, add, Username} ->
			...
			case maps:find(Username,RankingScore) of
				error ->  NewRankingScore = maps:put(Username,0,RankingScore),
							state(NewOnline,Socket,GreenMonsters,RedMonsters,ScoreInGame,NewRankingScore);
				_ -> state(NewOnline,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore)
			end;	
				...
				true -> 
						case check_boundaries_players(maps:to_list(Online)) of
							...
							{game_over, LostUsername} ->
								...
								NewRankingScore = maps:put(LostUsername,TOPLoser,RankingScore),
