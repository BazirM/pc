...
statelogin(Online,Socket,GreenMonsters,RedMonsters,RankingScore,RankingLevel) ->
...
% RedMonsters
state(Online,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel) ->
	...
	receive
		{online, add, Username} ->
			...
			case maps:find(Username,RankingScore) of
				error ->  NewRankingScore = maps:put(Username,0,RankingScore),
						  ...
						  state(NewOnline,Socket,GreenMonsters,RedMonsters,ScoreInGame,NewRankingScore,NewRankingLevel);
				_ -> state(NewOnline,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel)
			end;
		...
		{time,Sock,Username} ->
			...
			state(Online,[Sock | Socket],GreenMonsters,RedMonsters,NewScoreInGame,RankingScore,RankingLevel);
		...
		{left,Username} ->
			case maps:is_key(Username,Online) of
				...
				true ->
					...
					state(O,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel)
			end;
				...
				true -> 
						case check_boundaries_players(maps:to_list(Online)) of
							...
							{game_over, LostUsername} ->
								...
								notifyRed ! stop,
				              	state(#{},[],#{},#{},#{},NewRankingScore2,NewRankingLevel2)
						end
			end;
		...
		{generate_monsters} ->
				...
				state(Online,Socket,GM2,RM,ScoreInGame,RankingScore,RankingLevel);
		...
		{red_create, From} ->
			...
			state(Online,Socket,GreenMonsters,RM,ScoreInGame,RankingScore,RankingLevel);
                ...
                ++ float_to_list(Y) ++ " " ++ integer_to_list(Type) ++ "\n")) || Sock <- Socket, {I,{Speed,X,Y,H,W,DirX,DirY,Type}} <- NRM],
              					case check_collision_GreenMonsters(maps:to_list(Online),GreenMonsters) of 
              						{game_upt_continue} -> 
              							...
              							From ! {repeat},
              							state(Online,Socket,NewGreenMonsters,NewRedMonsters,ScoreInGame,RankingScore,RankingLevel);
              						...
              						{charge,Username,IM} ->
                						...
                						[gen_tcp:send(Sock,list_to_binary(Data3)) || Sock <- Socket],
              							From ! {repeat},
              							state(O,Socket,GM,NewRedMonsters,ScoreInGame,RankingScore,RankingLevel)
              					end;
