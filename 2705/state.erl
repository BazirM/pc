...
start(NumGameRoom, PidServer) ->  %Online, Socket, GreenMonsters, RedMonsters, ScoreInGame,RankingScore, RankingLevel
...
statelogin(Online,Socket,GreenMonsters,RedMonsters,RankingaScore,RankingLevel) ->
  ...
  %case maps:to_list(RankingLevel) of
  %	[] -> none;
  %	TopRankingLevel ->
  %	 [gen_tcp:send(Socket,list_to_binary("ranking_level " ++ Username ++ " " ++ integer_to_list(Level) ++ "\n"))
  %              || {Username,Level} <- TopRankingLevel]
  %end.
%Online #{Username => Avatar = ({Speed,Dir,X,Y,H,W})}
% GreenMonsters #{I => Monster = ({Speed,X,Y,H,W,Type})}
% RedMonsters
state(NumGameRoom,Online,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel) ->
	...
	receive
		{online, add, Username} ->
			...
			case maps:find(Username,RankingScore) of
				error ->  NewRankingScore = maps:put(Username,0,RankingScore),
						  NewRankingLevel = maps:put(Username,1,RankingLevel),
						  state(NumGameRoom, NewOnline,Socket,GreenMonsters,RedMonsters,ScoreInGame,NewRankingScore,NewRankingLevel);
				_ -> state(NumGameRoom, NewOnline,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel)
			end;
		...
		{time,Sock,Username} ->
			...
			statelogin(Online,Sock,GreenMonsters,RedMonsters,RankingScore,RankingLevel),
			state(NumGameRoom,Online,[Sock | Socket],GreenMonsters,RedMonsters,NewScoreInGame,RankingScore,RankingLevel);

		{left,Username, Charge} ->
			case maps:is_key(Username,Online) of
				false -> state(NumGameRoom, Online,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel);
				true ->
					...
					state(NumGameRoom, O,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel)
			end;
				...
				true -> 
						case check_boundaries_players(maps:to_list(Online)) of
							...
							{game_over, LostUsername} ->
								...
								LS = maps:get(LostUsername,RankingScore),
								WS = maps:get(WinnerUsername,RankingScore),
								TOPLoser = max(LS,Score),
								TOPWinner = max(WS,Score*2),
								NewRankingScore = maps:update(LostUsername,TOPLoser,RankingScore),
								...
								ListNewRankingScore2 = maps:to_list(NewRankingScore2),

								[gen_tcp:send(Sock,list_to_binary("ranking_score " ++ UsernameS ++ " " ++ integer_to_list(ScoreS) ++ "\n"))
                 						|| {UsernameS,ScoreS} <- ListNewRankingScore2, Sock <- Socket],

                 				ListNewRankingLevel2 = maps:to_list(NewRankingLevel2),

  	 							[gen_tcp:send(Sock,list_to_binary("ranking_level " ++ UsernameL ++ " " ++ integer_to_list(LevelL) ++ "\n"))
                 					|| {UsernameL,LevelL} <- ListNewRankingLevel2, Sock <- Socket],

                 				%[gen_tcp:send(Sock, list_to_binary("score_info " ++ WinnerUsername ++ " "                 					++ integer_to_list(TOPWinner) ++  " " ++ LostUsername ++ " "
                 				%	++ integer_to_list(TOPLoser) ++ "\n")) || Sock <- Socket],

                 				[gen_tcp:send(Sock, list_to_binary("score_info " ++ WinnerUsername ++ " "
                 					++ integer_to_list(TOPWinner) ++  " " ++ LostUsername ++ " "
                 					++ integer_to_list(TOPLoser) ++ "\n")) || Sock <- Socket],

                 				[gen_tcp:send(Sock, list_to_binary("level_info " ++ WinnerUsername ++ " "
                 					++ integer_to_list(WL+1) ++  " " ++ LostUsername ++ " "
                 					++ integer_to_list(LL) ++ "\n")) || Sock <- Socket],

                 				%[gen_tcp:send(Sock, list_to_binary("level_info " ++ "Winner: " ++ WinnerUsername ++ " Level: " 
                 				%	++ integer_to_list(WL) ++ " | " ++ "Loser " ++ LostUsername ++ " Level: "
                 				%	++ integer_to_list(LL) ++ "\n")) || Sock <- Socket],

								wait ! {game_over, WinnerUsername},
								notifyRed ! stop,
				              	state(NumGameRoom,#{},[],#{},#{},#{},NewRankingScore2,NewRankingLevel2)
						end
			end;

		{autocharge,Username,Energy} -> 
			case charge_time_energy(Username,Energy,Online) of
				{failure} -> state(NumGameRoom,Online,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel);
				{full} -> state(NumGameRoom,Online,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel);
				{NewOnline,Message} ->
					[gen_tcp:send(Sock,list_to_binary(Message)) || Sock <- Socket],
					state(NumGameRoom,NewOnline,Socket,GreenMonsters,RedMonsters,ScoreInGame,RankingScore,RankingLevel)
			end;
		...
		{generate_monsters, GRoom} ->
				...
				state(NumGameRoom,Online,Socket,GM2,RM,ScoreInGame,RankingScore,RankingLevel);
		...
		{red_create, From} ->
			...
			state(NumGameRoom,Online,Socket,GreenMonsters,RM,ScoreInGame,RankingScore,RankingLevel);
                ...
                ++ float_to_list(Y) ++ " " ++ integer_to_list(Type) ++ "\n")) || Sock <- Socket, {I,{Speed,X,Y,H,W,DirX,DirY,Type}} <- NRM],
              					case check_collision_GreenMonsters(maps:to_list(Online),GreenMonsters) of 
              						{game_upt_continue} -> 
              							...
              							From ! {repeat},
              							state(NumGameRoom,Online,Socket,NewGreenMonsters,NewRedMonsters,ScoreInGame,RankingScore,RankingLevel);
              						...
              						{charge,Username,IM} ->
                						...
                						[gen_tcp:send(Sock,list_to_binary(Data3)) || Sock <- Socket],
              							From ! {repeat},
              							state(NumGameRoom,O,Socket,GM,NewRedMonsters,ScoreInGame,RankingScore,RankingLevel)
              					end;
              		...
              		io:format("Game_OVER Red Monster!~n"),
								...
								LS = maps:get(Username,RankingScore),
								WS = maps:get(WinnerUsername,RankingScore),
								TOPLoser = max(LS,Score),
								TOPWinner = max(WS,Score*2),
								NewRankingScore = maps:put(Username,TOPLoser,RankingScore),
								%NewRankingScore2 = maps:update_with(WinnerUsername,fun(V) -> V+1 end,Score,NewRankingScore),
								NewRankingScore2 = maps:update(WinnerUsername,TOPWinner,NewRankingScore),
								...
								[gen_tcp:send(Sock,list_to_binary("ranking_score " ++ UsernameS ++ " " ++ integer_to_list(ScoreS) ++ "\n"))
                 				...
                 				[gen_tcp:send(Sock, list_to_binary("score_info " ++ WinnerUsername ++ " "
                 					++ integer_to_list(TOPWinner) ++  " " ++ Username ++ " "
                 					++ integer_to_list(TOPLoser) ++ "\n")) || Sock <- Socket],

                 				[gen_tcp:send(Sock, list_to_binary("level_info " ++ WinnerUsername ++ " "
                 					++ integer_to_list(WL+1) ++  " " ++ Username ++ " "
                 					++ integer_to_list(LL) ++ "\n")) || Sock <- Socket],
