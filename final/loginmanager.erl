...
logout(Username,Socket) ->
	?MODULE ! {logout,Username,self(),Socket},
	receive
		{?MODULE,Result} -> Result
	end.
...
management(Users,Onlines) ->
	receive
		...
		{login,Username,Passwd,From,Socket} ->
			case maps:find(Username,Users) of
				{ok,{Passwd,false}} -> From ! {?MODULE,ok, maps:size(Onlines)+1 },
		...
		{logout,Username,From,_} ->
			case maps:find(Username,Users) of
				{ok,{_Passwd,true}} -> From ! {?MODULE,ok},
									O = maps:remove(Username,Onlines),
									U = maps:update(Username,{_Passwd,false},Users),
									management(U,O);
				_ -> management(Users,Onlines)
			end;
